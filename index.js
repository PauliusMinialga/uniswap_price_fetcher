...
const { Pair, Route } = require('@uniswap/v2-sdk');
...
const getUniswapV2Price = async () => {
    ...
    // Define tokens
    const BTC = new Token(chainId, '0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599', 8, 'WBTC', 'Wrapped BTC');
    const ETH = WETH9[chainId];
    ...
    try {
        ...
        const pairAddress = Pair.getAddress(BTC, ETH);
        ...
        const btcPair = new Pair(CurrencyAmount.fromRawAmount(token0, reserve0), CurrencyAmount.fromRawAmount(token1, reserve1));
        
        if (!btcPair) {
            throw new Error('Failed to fetch pair data.');
        }

        console.log('Pair data fetched successfully.');
        const route = new Route([btcPair], BTC, ETH);

        console.log(`1 BTC = ${route.midPrice.toSignificant(6)} ETH`);
        console.log(`1 ETH = ${route.midPrice.invert().toSignificant(6)} BTC`);
    } catch (error) {
